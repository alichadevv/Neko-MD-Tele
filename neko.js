...
module.exports = neko = async (neko, bot) => {
    //console.log(neko)
    try {
        const body = neko.message.text || neko.message.caption || ''
        const budy = (typeof neko.message.text == 'string' ? neko.message.text : '')
        const { isUrl } = neko_bot;
        const isCmd = /^[°•π÷×¶∆£¢€¥®™�✓_=|~!?#/$%^&.+-,\\\©^]/.test(body)
        const prefix = isCmd ? body[0] : ''
        const command = isCmd ? body.slice(1).trim().split(' ').shift().toLowerCase() : ''
        const args = body.trim().split(/ +/).slice(1)
        const text = q = args.join(" ")
        const user = neko_bot.getUserName(neko.message.from)
        const pushname = user.full_name;
        const user_id = neko.message.from.id + " "
        const username = neko.message.from.username ? neko.message.from.username : "unknown";
        const isCreator = OWNER[0].replace("https://t.me/", '') == neko.update.message.from.username
        const from = neko.message.chat.id
        const opts = { parse_mode: 'MARKDOWN' };

        //Groups
        const isGroup = neko.chat.type.includes('group')
        const groupName = isGroup ? neko.chat.title : ''
        const newMember = neko.message.new_chat_member;
        const leftMember = neko.message.left_chat_member;

        const isBanned = banned.includes("" + from)
        const isImage = neko.message.hasOwnProperty('photo')
        const isVideo = neko.message.hasOwnProperty('video')
        const isAudio = neko.message.hasOwnProperty('audio')
        const isSticker = neko.message.hasOwnProperty('sticker')
        const isContact = neko.message.hasOwnProperty('contact')
        const isLocation = neko.message.hasOwnProperty('location')
        const isDocument = neko.message.hasOwnProperty('document')
        const isAnimation = neko.message.hasOwnProperty('animation')
        const isMedia = isImage || isVideo || isAudio || isSticker || isContact || isLocation || isDocument || isAnimation
        const quotedMessage = neko.message.reply_to_message || {}
        ...
        const isQuoted = neko.message.hasOwnProperty('reply_to_message')
        ...
        switch (command) {
            ...
            case "menu": case "help": {
                if (isBanned) return reply(LANGUAGE_IND.mess.banned)
                LANGUAGE_IND.menu(neko, THUMBNAIL, pushname, OWNER_NAME, OWNER, prefix, latensii, os, neko_bot, username, signup.length, isCreator, user.id.toString())
            }
            break
            case "donasi": case "donate": {
                LANGUAGE_IND.donasi(neko, DONASI)
            }
            break
            case "script": {
                if (isBanned) return reply(LANGUAGE_IND.mess.banned)
                neko.reply("Source code this bot...", {
                    reply_markup: {
                        inline_keyboard: [
                            [{
                                text: 'Github',
            ...
            case "ping": case "botsatus": {
                ...
                respon = `_Kecepatan Respon_ ${latensi.toFixed(4)} _Second_ \n ${oldd - neww} _miliseconds_\n_Runtime_ : ${neko_bot.runtime(process.uptime())}\n\n💻 _Server Information_\nRAM: ${neko_bot.formatp(os.totalmem() - os.freemem())} / ${neko_bot.formatp(os.totalmem())}\n_NodeJS Memory Usaage_\n${Object.keys(used).map((key, _, arr) => `${key.padEnd(Math.max(...arr.map(v=>v.length)),' ')}: ${neko_bot.formatp(used[key])}`).join('\n')}\n\n${cpus[0] ? `_Total CPU Usage_\n${cpus[0].model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}\n_CPU Core(s) Usage (${cpus.length} Core CPU)_\n\n${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}`.trim()
                reply(respon)
            }
            break
            case "anime": case "waifu": case "husbu": case "neko": case "shinobu": case "megumin": {
                ...
                const image_url = api('btc', '/randomanime/' + command, {}, 'apikey')
                var button = [
                    [{
                        text: 'Next ⏭',
            ...
            case "restart": {
                ...
                try{
                    ...
                    await neko_bot.sleep(3000)
                    exec(`npm start`)
                } catch (err) {
                    exec(`node index.js`)
                    await neko_bot.sleep(4000)
                    reply('Sukses')
                }
            }
            break
            case "gempa": {
                ...
                let anu = await neko_bot.fetchJson('https://data.bmkg.go.id/DataMKG/TEWS/autogempa.json')
                neko.replyWithPhoto({
            ...
            case "tiktoknowm": case "tiktok": {
                ...
                reply(LANGUAGE_IND.mess.wait)
                let anu = await neko_bot.fetchJson(api('btc', '/downloader/tiktok', { url: args[0] }, 'apikey'))
            ...
            case "darkjoke": case "memeindo": case "meme": case "patrick": case "profil": {
                if (isBanned) return reply(LANGUAGE_IND.mess.banned)
                reply(LANGUAGE_IND.mess.wait)
                const image_url = api('btc', '/randomimage/' + command, {}, 'apikey')
                var button = [
                    [{
                        text: 'Next ⏭',
                        callback_data: command + '*' + user_id
            ...
            case "diffusion" : {
                ...
                reply(LANGUAGE_IND.mess.wait)
                const image_url = api('lann', '/api/diffusion-prompt', {prompt: text}, 'apikey')
                neko.replyWithPhoto({
                    url: image_url,
                }, {
                    caption: `_${text}_`,
            ...
            case "whoisip": {
                ...
                reply(LANGUAGE_IND.mess.wait)
                try {
                    let anu = await neko_bot.fetchJson(api('lann', '/api/ipaddress/' + args[0], {}, 'apikey'))
            ...
            case "shortlink": {
                ...
                let anu = await neko_bot.fetchJson(api('lann', '/api/shortlink', {url: args[0]}, 'apikey'))
            ...
            case "getcase": {
                ...
                try {
                    reply(LANGUAGE_IND.mess.wait)
                    let code = "case " + `"${args[0]}"` + fs.readFileSync('./nekobot.js').toString().split(`case "${args[0]}"`)[1].split('break')[0] + 'break'
                    let image_url = api('lann', '/api/carbon', { code: code, language: 'javascript' }, 'apikey')
                    neko.replyWithPhoto({
                        url: image_url
                    }, {
            ...
            case "ytmp3": {
                ...
                reply(LANGUAGE_IND.mess.wait)
                let anu = await fetch(api('lann', '/api/ytaudio', { url: args[0] }, 'apikey'))
            ...
            case "ytmp4": case "ytshorts": {
                ...
                if (command === 'ytshorts') {
                    reply(LANGUAGE_IND.mess.wait)
                    let anu = await fetch(api('lann', '/api/ytvideo2', { url: args[0] }, 'apikey'))
                    if (!anu.ok) throw await anu.text()
                    var result = await anu.json()
                    ...
                    reply(LANGUAGE_IND.mess.wait)
                    let anu = await fetch(api('lann', '/api/ytvideo', { url: args[0] }, 'apikey'))
                    if (!anu.ok) throw await anu.text()
                    var result = await anu.json()
            ...
            case "toaudio": {
                ...
                reply(LANGUAGE_IND.mess.wait)
                let anu = (api('lann', '/api/gtts/id', { text: text }, 'apikey'))
                neko.replyWithAudio({
                    url: anu,
            ...
            case "stalkgithub": case "stalkinstagram": case "stalktiktok": case "stalktwitter": {
                ...
                if (command === 'stalkgithub') {
                    reply(LANGUAGE_IND.mess.wait)
                    let anu = await fetch(api('lann', '/api/github/' + text, {}, 'apikey'))
                    if (!anu.ok) throw await anu.text()
                    var result = await anu.json()
                    var {avatar, url, name, followers, following, public_repos, public_gists, type, company, location, email, bio} = result.result
                    var get_link = await neko_bot.fetchJson(`https://tinyurl.com/api-create.php?url=${url}`)
                    ...
                    reply(LANGUAGE_IND.mess.wait)
                    let anu = await fetch(api('lann', '/api/stalkig/' + text, {}, 'apikey'))
                    if (!anu.ok) throw await anu.text()
                    var result = await anu.json()
                    ...
                    reply(LANGUAGE_IND.mess.wait)
                    let anu = await fetch(api('lann', '/api/stalktiktok/' + text, {}, 'apikey'))
                    if (!anu.ok) throw await anu.text()
                    var result = await anu.json()
                    ...
                    reply(LANGUAGE_IND.mess.wait)
                    let anu = await fetch(api('lann', '/api/twitter/' + text, {}, 'apikey'))
                    if (!anu.ok) throw await anu.text()
                    var result = await anu.json()
            ...
            case "apkdownload": {
                ...
                reply(LANGUAGE_IND.mess.wait)
                let anu = await fetch(api('lann', '/api/apkdownloader', {package: text }, 'apikey'))
                if (!anu.ok) throw await anu.text()
                var result = await anu.json()
                var { apk_name, apk_icon, apk_version, apk_author, apk_link } = result.result
                var get_link = await neko_bot.fetchJson(`https://tinyurl.com/api-create.php?url=${apk_link}`)
            ...
            case "mediafire": {
                ...
                reply(LANGUAGE_IND.mess.wait)
                let anu = await fetch(api('lann', '/api/mediafire', {url: args[0] }, 'apikey'))
                if (!anu.ok) throw await anu.text()
                var result = await anu.json()
            ...
            case "cuaca": {
                ...
                reply(LANGUAGE_IND.mess.wait)
                let anu = await fetch(api('lann', '/api/cuaca/' + text, {}, 'apikey'))
                if (!anu.ok) throw await anu.text()
                var result = await anu.json()
            ...
            case "nulis": {
                ...
                if (!text) return reply(`Example: ${prefix + command} Hello World`)
                reply(LANGUAGE_IND.mess.wait)
                let anu = await api('lann', '/api/nulis', {text: text }, 'apikey')
                neko.replyWithPhoto({
                    url: anu
                }, {
                    caption: LANGUAGE_IND.mess.success,
            ...
            case "translate": {
                ...
                if (!text) return reply(`Example: ${prefix + command} Hello World`)
                reply(LANGUAGE_IND.mess.wait)
                let anu = await fetch(api('lann', '/api/translate/auto/id', {text: text }, 'apikey'))
                if (!anu.ok) throw await anu.text()
                var result = await anu.json()
            ...
            case "fetch": {
                ...
                async function fetchData() {
                    try {
                        ...
                        let file_name = neko_bot.getRandom("json")
            ...
            case "buysourcecode": {
                neko.replyWithPhoto({
                    ...
                    caption: '_Untuk melakukan pembelian contact_ [Click Here](https://wa.me/+6281400346604)',
                    parse_mode: "MARKDOWN"
                })
            }
            //break
            ...
            case "qrcode": {
                ...
                if (!text) return reply(`Example: ${prefix}${command} Hello World`)
                reply(LANGUAGE_IND.mess.wait)
                let url = api('lann', '/api/qrcode', {text: text }, 'apikey')
                neko.replyWithPhoto({
                    url: url
                }, {
                    caption: LANGUAGE_IND.mess.success,
